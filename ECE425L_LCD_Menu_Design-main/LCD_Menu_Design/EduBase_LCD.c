...
void EduBase_LCD_Ports_Init(void)
{
	...
	// R0 bit (Bit 0) in the RCGRGPIO register
	SYSCTL -> RCGCGPIO |= 0x01;
	
	//Configrue hte PA5, OA4, PA3, and PA2 pins as output
	// by setting Bits 5 to 2 in the DIR register
	GPIOA -> DIR |= 0x3C;
	
	//Configre the PA5, PA4, PA3 and PA2  pins to function as
	//GPIO pins by clearing Bits 5 to 2 in the AFSEL register
	GPIOA -> AFSEL &= ~0x3C;
	
	//Enable the gigital functinality for the PA5, PA4, PA3 and PA2 pins
	// by setting Bits 5 to 2 in the DEN register
	GPIOA -> DEN |= 0x3C;
	
	//Initialize the output of the PA5, PA4, PA3 and PA2 pins to zero
	// by clearing Bits 5 ot 2 in the DATA register
	GPIOA -> DATA &= ~0x3C;
	
	//Enable the clock to Port  by setting the 
	// R2 bit (Bit 2) in the RCGCGPIO register
	SYSCTL -> RCGCGPIO |= 0x04;
	
	//Configrue the PC6 pins as output by setting Bit 6 in the DIR register
	GPIOC -> DIR |= 0x40;
	
	//Configure the PC6 pin to functon as a GPIO pin
	// by clearing Bit 6 in the AFSEL register
	GPIOC -> AFSEL &= ~0x40;
	
	//Enablle the digital functionality for the PA6 pin
	// by setting Bit 6 in the DEN register
	GPIOC -> DEN |= 0x40;
	
	//Initialize the output of the PC6 pins to zero
	...
	//Configure the PE0 pin as output by setting Bit 0 in thr Dir register
	GPIOE -> DIR |= 0x01;
	
	//Configure the PE0 pin to fucntion as aGPIO pin
	// by clearing Biy 0 in the AFSEL register
	GPIOE -> AFSEL &= ~0x01;
...
// by clearing Bit 0 in teh DATA register
GPIOE -> DATA &= ~0x01;
}
...
void EduBase_LCD_Pulse_Enable(void)
{
	...
	//ENsure that the output of the PC6 pin is zero befroe sending a short pulse
	GPIOC -> DATA &= ~0x40;
	SysTick_Delay1us(1);
	
	//Output a short pulse on the PC6 pin by setting Bit 6
	//in the DATA register high and clearing it after 1 us
	//The minimum timer for  the enable pulse width must be at least greater than 40 ns
	// during a read / write operation (page 49 of th HD44780 datasheet)
	GPIOC-> DATA |= 0x40;
	SysTick_Delay1us(1);
	GPIOC->DATA &= ~0x40;
}
...
void EduBase_LCD_Write_4_Bits(uint8_t data, uint8_t control_flag)
{
	...
	//Set the upper niblle of the data on the data pins (PA2 - PA5)
	GPIOA -> DATA |= (data & 0xF0) >> 0x2;
	
	//Ser or clear the register select (RS) pin based on the control flag
...
void EduBase_LCD_Send_Command(uint8_t command)
{
	...
	// The first two command required 1.52 ms executed time
	// while the rest of the command need 37us
	if(command < 3)
	{
...
void EduBase_LCD_Send_Data(uint8_t data)
{
	...
	//Transmitt the upper niblle of the data byte
...
{
	//Initialize the GPIO pins used by the LED
	EduBase_LCD_Ports_Init();
	
	//Provide a delay of 50ms after the LCD is powered on
	SysTick_Delay1us(50000);
	
	//Transmit the functino set intialization commands as part of the LCD intialization sequence
	...
	//Transmitt  a function set comman to the LCD to configure it to use  40bits mode
	...
	//Transmtt a Return Home command to set the DDRAM address to 0 and to return
	// the display from being shfted to the orginal position
	EduBase_LCD_Return_Home();
	
	//Transmitt a Clear Display command to clear the display and set the DDRAM address to 0
